...
const CategoryTrashCan = () => {
    const [categories, setCategories] = useState([]);
    const [deletingId, setDeletingId] = useState(null);
    const [RestoreId, setRestoreId] = useState(null);
    const fetchCategories = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/api/categoryIsDelete`);
    ...
    const handleRestore = (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const token = localStorage.getItem("token");
                axios.put(
                    `http://localhost:3000/api/category/${id}/restore`,
                                    ...
                                    {categories.map((cate) => (
                                        <tr key={cate.id}>
                                            ...
                                            <td>
                                                <Button
                                                    variant="primary"
                                                    onClick={() => handleRestore(cate.id)}
                                                    disabled={RestoreId === cate.id}
                                                    className="me-2"
                                                >
                                                ...
                                                <Button variant="danger" onClick={() => deleteCategory(cate.id)}>
...
export default CategoryTrashCan